...
// 		fprintf(stdout,"y: %i %i %i 1\n",rp->y, x, y);
 			...
 			rp->pix[p]=(unsigned char)((unsigned int)(rp->pix[p] + rp->tint[0]) / 3);
 			rp->pix[p+1]=(unsigned char)((unsigned int)(rp->pix[p+1] + rp->tint[1]) / 3);
 			rp->pix[p+2]=(unsigned char)((unsigned int)(rp->pix[p+2] + rp->tint[2]) / 3);
 			rp->pix[p+3]=0xff;
#ifdef DEBUG
